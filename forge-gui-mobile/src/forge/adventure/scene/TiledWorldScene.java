 ...
 */
public class TiledWorldScene extends HudScene   {
    List<TiledMap> maps;
    ...
    private static TiledWorldScene object;

    public static TiledWorldScene instance() {
        if(object==null)
            object=new TiledWorldScene();
        return object;
    }
    ...
    public void load(PointOfInterest point) {
        ...
        TiledMap map = new TemplateTmxMapLoader().load(Config.instance().getCommonFilePath(point.getData().map));
    ...
    private void load(String targetMap, int nextSpawnPoint) {
        TiledMap map = new TemplateTmxMapLoader().load(Config.instance().getFilePath(targetMap));
